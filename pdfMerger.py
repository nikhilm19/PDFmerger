...
class merge(Tk):
    ...
    def selectFiles(self):
        self.newFiles=list((filedialog.askopenfilenames(title="Select Files",filetypes=(("Pdf files", "*.pdf"),))))
        self.files+=self.newFiles
        self.list.delete(5)
        for i in range(len(self.files)):
            self.list.insert(i,str(os.path.split(self.files[i])[1]))
    def createWidgets(self):
        ...
        self.developer.pack(expand=True,fill="x")
        self.selectLabel=Label(self.box,text="Selected files",font=("Arial",12,"italic"),width=20,bd=0,height=2,bg="light green",anchor="w",highlightbackground="grey",highlightthickness=1)
        self.selectLabel.pack(fill="x")
        self.list=Listbox(self.box,bg="grey",selectmode=MULTIPLE,fg="white")
        #self.list.insert(5,"No files selected")
